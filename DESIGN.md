...
| On Primary | `#ffffff` | `--color-on-primary` | primary fill 위 텍스트 |
...
| Warning Border | `#ffe08a` | `--color-warning-border` | warning weak background과 짝 border |
| Danger Border | `#ffc3c8` | `--color-danger-border` | danger weak background과 짝 border |
| Success Border | `#b8ebd5` | `--color-success-border` | success weak background과 짝 border |
| Blue Border | `#b8d9ff` | `--color-blue-border` | blue weak background과 짝 border |
...
다크모드는 기본적으로 OS `prefers-color-scheme: dark`를 따르고, 사용자가 theme toggle을 누르면 `html[data-theme]`의 명시 선택이 우선한다. 전체 canvas는 사용자의 요청에 맞춰 `#000000`을 쓰고, surface hierarchy는 기존 grey palette를 뒤집어 만든다. feature code는 light/dark 분기 없이 semantic token만 참조한다.

| Role | Dark Value | Token |
|------|------------|-------|
| Background | `#000000` | `--color-background` |
| Surface | `var(--tds-grey-900)` | `--color-surface` |
| Raised Surface | `var(--tds-grey-800)` | `--color-surface-raised` |
| Border | `color-mix(... --tds-grey-800 ... #000000)` | `--color-border` |
| Strong Border | `var(--tds-grey-700)` | `--color-border-strong` |
| Primary Text | `var(--tds-grey-50)` | `--color-text-primary` |
| Secondary Text | `var(--tds-grey-300)` | `--color-text-secondary` |
| Tertiary Text | `var(--tds-grey-400)` | `--color-text-tertiary` |
| Primary Action | `var(--tds-grey-50)` | `--color-primary` |
| On Primary | `#000000` | `--color-on-primary` |
| Primary Weak | `var(--tds-grey-800)` | `--color-primary-weak` |
| Warning | `var(--tds-yellow-500)` | `--color-warning` |

Status weak backgrounds and borders in dark mode are `color-mix()` blends of the same accent token and `--color-background`; do not reintroduce pastel light-mode weak colors directly on dark surfaces.
...
CSS 변수 `--text-t1`부터 `--text-t9`만 사용한다. 700 weight는 예외적 강조에만 쓴다. UI scale 외 장식용 디스플레이 사이즈가 필요한 경우 `--text-display-fortune` 같은 명시적 토큰을 추가한다.
...
| display-fortune | 44px | 1 | 0 | `--text-display-fortune` |
...
Sticky top, 52px height, translucent semantic surface, bottom border. Mobile에서는 wrapping보다 horizontal scroll을 우선한다. Active tab은 `aria-current="page"`와 neutral weak background로 표시한다. Durable section은 popover가 아니라 route/tab으로 노출한다.

### Surface And Section
**Role:** Main content grouping for search, results, forecasts, fortune, and release notes.

Use `border: 1px solid var(--color-border)`, `background: var(--color-surface)`, `border-radius: var(--radius-surface)`. Ordinary card padding은 16px, roomy card는 20px까지 허용한다. Surface 안에 nested card를 만들지 않고, 내부 구분은 `border-top`, `divide-y`, table row border로 처리한다. 소개와 보조 탭의 full-page 카드는 semantic surface, 8px radius, 내부 divider가 유지될 때 외곽 border를 생략할 수 있다. 데이터와 결과 surface는 기본 border를 유지한다.
...
우측 하단 ticker tape 바로 위에 fixed icon-only button으로 둔다. Toss Mini TDS IconButton의 형태를 참고해 44px touch target, 20px mono icon, semantic raised surface를 48%로 반투명 처리한 borderless glass surface, backdrop blur를 사용한다. 메뉴바 공간을 침범하지 않으며, 버튼에는 visible text를 넣지 않고 `aria-label`, `aria-pressed`, `title`을 현재 전환 대상에 맞게 갱신한다. 사용자가 직접 선택한 light/dark 값은 `localStorage`에 저장하고, 명시 선택이 없을 때만 OS `prefers-color-scheme`을 따른다.
...
- 라이트 모드에서는 밝은 work surface를 기본으로 유지한다: `#f2f4f6` 배경, 흰 surface, 얇은 회색 border.
- 다크 모드에서는 `#000000` canvas, grey-900 surface, grey-800 raised surface 위계를 유지한다.
...
|-------|------|-------------|------------|---------|
| 0 | Work Canvas | `#f2f4f6` | `#000000` | 전체 app background |
| 1 | Main Surface | `#ffffff` | `var(--tds-grey-900)` | 주요 카드, header, table cell |
| 2 | Raised Surface | `#ffffff` | `var(--tds-grey-800)` | dropdown, overlay panel |
| 3 | Weak Neutral | `#f2f4f6` | `var(--tds-grey-800)` | selected tab, hover, weak grouping |
| 4 | Status Weak Areas | pastel weak tokens | dark `color-mix()` weak tokens | success, danger, warning, info 상태 배경 |

## Elevation

근형봇은 elevation을 shadow가 아니라 surface color, border, divider, spacing으로 만든다. 일반 카드 shadow는 금지하고, `--shadow-focus`는 focused control에만 사용한다. Dropdown과 overlay는 `--color-surface-raised`와 `--color-border-strong`으로 구분한다. Borderless page-card shell은 외곽 stroke 대신 surface color, radius, spacing, 내부 divider로 구분한다.
...
현재 DOM과 JS는 `.warning-terminal`, `.tm-*`, `.forecast-*`, `.patch-*` 클래스를 사용한다. 이 이름은 호환 layer로 유지하지만, 시각 언어는 semantic 디자인 토큰을 따른다. 과거에 존재하던 `--tm-*`, `--fs-*`, `--mono`, `--font-display` 등 alias 토큰은 모두 제거됐고 새 코드에서 직접 사용하지 않는다.
...
- Dark Background: `#000000` via `--color-background`
- Dark Surface: `#191f28` via `--color-surface`
...
:root {
  ...
  --color-danger-border: #ffc3c8;
  --color-success-border: #b8ebd5;
  --color-blue-border: #b8d9ff;
...
:root[data-theme="dark"] {
  ...
  --color-surface: var(--tds-grey-900);
  --color-surface-raised: var(--tds-grey-800);
  --color-text-primary: var(--tds-grey-50);
  --color-text-secondary: var(--tds-grey-300);
  --color-text-tertiary: var(--tds-grey-400);
  --color-primary: var(--tds-grey-50);
  --color-on-primary: #000000;
  --color-primary-weak: var(--tds-grey-800);
}

/* JS mirrors OS or saved user preference into html[data-theme]. */
```
...
- 라이트 배경은 `#f2f4f6`, 다크 배경은 `#000000` 토큰을 따른다.
