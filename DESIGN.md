...
현재 DOM과 JS는 `.warning-terminal`, `.tm-*`, `.forecast-*`, `.patch-*` 클래스를 사용한다. 이 이름은 호환 layer로 유지하지만, 시각 언어는 밝은 디자인 토큰을 따른다. 과거에 존재하던 `--tm-*`, `--fs-*`, `--mono`, `--font-display` 등 alias 토큰은 모두 제거됐고 새 코드에서 직접 사용하지 않는다.
